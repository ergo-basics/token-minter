<script lang="ts">
    ...
    async function handleMint() {
        ...
        try {
            ...
            errorMessage = err.info || err.message || "An unknown error occurred.";
        } finally {
            isLoading = false;
        }
    }
...
{#if $address}
    <Dialog.Root bind:open={showWalletInfo}>
        <Dialog.Content>
            <Dialog.Header><Dialog.Title>Wallet Info</Dialog.Title></Dialog.Header>
...
<Dialog.Root bind:open={showKyaModal}>
  <Dialog.Content class="w-[700px] max-w-[85vw] sm:max-w-[70vw]">
    ...
    <div bind:this={kyaContentDiv} on:scroll={checkKyaScroll} class="max-h-[50vh] overflow-y-auto pr-4 text-sm">
      ...
      <ul class="list-disc ml-6 space-y-2">
        <li><strong>"As-Is" Software:</strong> This tool is provided "as is," without warranties of any kind. There is no guarantee against errors or bugs. Use it at your own risk.</li>
        <li><strong>Irreversible Transactions:</strong> Once you sign and submit a transaction to mint tokens, it cannot be undone. Double-check all parameters before confirming.</li>
        <li><strong>Wallet Security:</strong> You are solely responsible for the security of your wallet, private keys, and any assets. This application never has access to your keys.</li>
        <li><strong>No Central Authority:</strong> As a decentralized tool, there is no central entity to appeal to for lost funds or failed transactions. You are interacting directly with a permissionless blockchain.</li>
      </ul>
      <p class="italic mt-6">Do you understand and accept these assumptions and the associated risks of using this tool?</p>
    </div>
    <Dialog.Footer class="mt-4">
      <Button on:click={handleCloseKyaModal} disabled={!isKyaButtonEnabled} class="w-full sm:w-auto">
...
<main class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto">
        ...
        <p class="text-muted-foreground mb-6">Complete the details below to create your own token on the Ergo blockchain.</p>
        
        {#if $connected}
            <form on:submit|preventDefault={handleMint} class="space-y-4">
                <div>
                    <Label for="tokenName">Token Name</Label>
                    <Input id="tokenName" bind:value={tokenName} placeholder="E.g., MyToken" required />
                </div>
                <div>
                    <Label for="amount">Amount</Label>
                    <Input id="amount" type="number" bind:value={amountStr} placeholder="E.g., 1000000" required />
                </div>
                <div>
                    <Label for="decimals">Decimals</Label>
                    <Input id="decimals" type="number" bind:value={decimalsStr} placeholder="0-8" required />
                </div>
                <div>
                    <Label for="description">Description</Label>
                    <Input id="description" bind:value={description} placeholder="A short description of my token" />
                </div>
            ...
            <div class="mt-6 text-center">
                {#if isLoading}
                    <p class="text-blue-500">Processing transaction. Please check your wallet to sign...</p>
                {/if}
                {#if transactionId}
                    <div class="p-4 bg-green-100 dark:bg-green-900 border border-green-400 rounded-md">
                        <p class="font-semibold text-green-800 dark:text-green-200">Mint successful!</p>
                        <p class="text-sm">Transaction ID:</p>
                        <a href={`https://explorer.ergoplatform.com/en/transactions/${transactionId}`} target="_blank" rel="noopener noreferrer" class="text-blue-600 dark:text-blue-400 break-all">{transactionId}</a>
                    </div>
                {/if}
                {#if errorMessage}
                     <div class="p-4 bg-red-100 dark:bg-red-900 border border-red-400 rounded-md">
...
<footer class="page-footer">
    <div class="footer-left">
        <span class="cursor-pointer hover:underline" on:click={handleOpenKyaModal} on:keydown={(e) => e.key === 'Enter' && handleOpenKyaModal()} role="button" tabindex="0">
            KYA
        </span>
    </div>
